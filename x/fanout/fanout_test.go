...
func (s *FanoutTestSuite) TestBulkFutureIsReady() {
	wf := func(ctx internal.Context) error {
		ctx = internal.WithActivityOptions(ctx, internal.ActivityOptions{
			ScheduleToStartTimeout: time.Minute,
			StartToCloseTimeout:    time.Second,
		})

		items := []string{"a", "b", "c"}
		future := fanout.ExecuteActivityInBulk[string, string](
			ctx,
			echoActivity,
			items,
			fanout.WithMaxConcurrentActivities(2),
		)
...
func (s *FanoutTestSuite) TestSimpleExecution() {
	wf := func(ctx internal.Context) error {
		...
		future := fanout.ExecuteActivityInBulk[string, string](
			...
			echoActivity,
			items,
			fanout.WithMaxConcurrentActivities(10),
		)
...
func (s *FanoutTestSuite) TestManyItems() {
	wf := func(ctx internal.Context) error {
		...
		future := fanout.ExecuteActivityInBulk[int, int](
			ctx,
			incrementActivity,
			items,
...
func (s *FanoutTestSuite) TestConcurrencyLimit() {
	wf := func(ctx internal.Context) error {
		...
		future := fanout.ExecuteActivityInBulk[int, int](
			...
			incrementActivity,
			items,
			fanout.WithMaxConcurrentActivities(3),
		)
...
func (s *FanoutTestSuite) TestWithOffsetLimit() {
	wf := func(ctx internal.Context) error {
		...
		future := fanout.ExecuteActivityInBulk[offsetLimitParams, offsetLimitResult](
			ctx,
			processChunkActivity,
			params,
			fanout.WithMaxConcurrentActivities(5),
		)
...
var retryAttempts = make(map[string]int)
...
func retryableActivity(ctx context.Context, input string) (string, error) {
	...
	if retryAttempts[input] <= maxRetries {
		return "", fmt.Errorf("attempt %d failed", retryAttempts[input])
	}
...
func (s *FanoutTestSuite) TestRealisticBulkEmail() {
	wf := func(ctx internal.Context, numEmails int) error {
		ctx = internal.WithActivityOptions(ctx, internal.ActivityOptions{
		...
		future := fanout.ExecuteActivityInBulk[emailRequest, emailResult](
			...
			sendEmailActivity,
			emails,
			fanout.WithMaxConcurrentActivities(50),
		)
...
func (s *FanoutTestSuite) TestRealisticCatalogBuild() {
	wf := func(ctx internal.Context, totalRecords int) error {
		ctx = internal.WithActivityOptions(ctx, internal.ActivityOptions{
			ScheduleToStartTimeout: time.Hour,
	...
	wf := func(ctx internal.Context) error {
		...
		future := fanout.ExecuteActivityInBulk[int, int](
			...
			fanout.WithFanoutOptions(fanout.FanoutOptions{
				...
				ChildExecutionTimeout:    time.Hour,
				ChildTaskTimeout:         10 * time.Second,
			}),
		)
