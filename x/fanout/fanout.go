...
type (
	...
	FanoutOptions struct {
		...
		// ChildExecutionTimeout is the ExecutionStartToCloseTimeout for each child workflow.
		ChildExecutionTimeout time.Duration

		// ChildTaskTimeout is the TaskStartToCloseTimeout for each child workflow.
		ChildTaskTimeout time.Duration
	}
...
func ExecuteActivityInBulk[T, R any](
	...
	if shouldFanout {
		return executeActivityInBulkWithChildren[T, R](ctx, activity, items, opts)
	}
	...
	for i, chunk := range chunks {
		...
		workflow.Go(ctx, func(ctx workflow.Context) {
			...
			childCtx := workflow.WithChildOptions(ctx, workflow.ChildWorkflowOptions{
				WorkflowID:                   childID,
				ExecutionStartToCloseTimeout: opts.FanoutOptions.ChildExecutionTimeout,
				TaskStartToCloseTimeout:      opts.FanoutOptions.ChildTaskTimeout,
				ParentClosePolicy:            client.ParentClosePolicyRequestCancel,
			})
...
func WithFanoutOptions(fanoutOpts FanoutOptions) Option {
	return func(opts *Options) {
		...
		if fanoutOpts.ChildExecutionTimeout <= 0 {
			fanoutOpts.ChildExecutionTimeout = time.Hour
		}
		if fanoutOpts.ChildTaskTimeout <= 0 {
			fanoutOpts.ChildTaskTimeout = 10 * time.Second
		}
